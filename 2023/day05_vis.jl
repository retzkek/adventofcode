...
# ╔═╡ c1b56420-96cd-11ee-2deb-27ce90548456
using CairoMakie

# ╔═╡ b73fe3a6-a5ae-448a-a0bc-454b15e7c583
using PlutoUI

# ╔═╡ 030efdd8-d5be-4f0e-9edb-2e456f5c7c03
...
# ╔═╡ 5d7f8838-a41d-4670-90b1-bb31abdd150d
ranges = reshape(seeds, (2, :))

# ╔═╡ a341c925-653d-4e87-a5d5-19fe00b21718
maps = [
...
# ╔═╡ 139a7a2c-84ba-4955-a36a-5ba689233b2a
md"""
seed: $(@bind iseed PlutoUI.Slider(1:5e9))
"""

# ╔═╡ 73adbb9b-5894-443d-82e3-1c7e4b055d0b
...
# ╔═╡ 322e4f47-8c8a-41e3-a9db-6cf3cff8629f
mappings = mapseed(iseed)

# ╔═╡ 6ac7dad3-45f2-4116-a191-0a0b73adcf5d
begin
	f = Figure()
	Axis(f[1, 1], yticks = (9:-1:2, ["seeds","soil","fertilizer","water","light","temperature","humidity","location"]))
	# seeds
	poly!([Rect(x[1], 9, x[2], 1) for x in eachcol(ranges)])
	# mappings
	y = 9
	for m in maps
    	for r in m
     	   poly!(Point2f[(r[1], y-1), (r[1]+r[3], y-1), (r[2]+r[3], y), (r[2], y)])
  		end
    	y -= 1
	end
	# line
	xs = [iseed; iseed; iseed; repeat(mappings[2:length(mappings)-1],inner=2); last(mappings)]
	ys = [10; repeat(9:-1:3,inner=2); 2]
	linesegments!(xs, ys, linewidth=2, color = :black)
	f
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "960ee8fa48364f270f25b963510b684bde1cafbb"
...
git-tree-sha1 = "fac89469860b22e1d710a711852397821ab313f3"
