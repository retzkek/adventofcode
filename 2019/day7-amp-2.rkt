...
(define-syntax-rule (op2 op prog args step modes out)
  (let* ([a (param (first modes) (second args) prog)]
         ...
         [dest (fourth args)])
    (exe (sub-at prog dest (op a b)) (+ step 4) out)))

(define (op-input prog args step modes out)
  #;(display "> ")
  (let ([v (thread-receive)])
    (exe (sub-at prog (second args) v) (+ step 2) out)))
...
(define (op-cmp f prog args step modes out)
  (let* ([a (param (first modes) (second args) prog)]
         ...
         [val (if (f a b) 1 0)]
         [dest (fourth args)])
    (exe (sub-at prog dest val) (+ step 4) out)))
...
(define (check-program prog newprog)
  (check-equal?
   (exe (read-program (open-input-string prog)) 0 (make-channel))
