defmodule AOC.Year23.Day10 do
  ...
  def trace_pipes(sketch) do
    ...
    trace_pipes(sketch, [start], hd(connections(sketch, start)))
  end

  def trace_pipes(sketch, path, node) do
    cons = connections(sketch, node)

    if length(path) > 1 && find_start(sketch) in cons do
      [node | path]
    else
      trace_pipes(sketch, [node | path], hd(Enum.reject(cons, fn x -> x in path end)))
    end
  end
